...
if [[ "$CHOICE" == "<< Back"* ]] || [[ -z "$CHOICE" ]]; then
	...
	INSTALLER_FILE=$(gum file $OMAKARCH_PATH/install)
	...
	case "$INSTALLER" in
	"dev-language") INSTALLER_FILE="$OMAKARCH_PATH/install/terminal/select-dev-language.sh" ;;
	"dev-database") INSTALLER_FILE="$OMAKARCH_PATH/install/terminal/select-dev-storage.sh" ;;
